...
static void ipc_client_message(struct snd_sof_dev *sdev, u32 msg_cmd);
...
{
	struct snd_sof_dev *sdev = snd_sof_get_sof_dev(&client->pdev->dev);
...
void snd_sof_ipc_msgs_rx(struct snd_sof_dev *sdev)
{
	...
	case SOF_IPC_GLB_COMP_MSG:
		break;
	case SOF_IPC_GLB_STREAM_MSG:
		ipc_client_message(sdev, hdr.cmd);
		break;
	case SOF_IPC_GLB_TRACE_MSG:
...
static void ipc_client_message(struct snd_sof_dev *sdev, u32 msg_cmd)
{
	...
	list_for_each_entry(sof_client, &sdev->ipc_rx_list, list) {
		if (sof_client->sof_client_rx_cb)

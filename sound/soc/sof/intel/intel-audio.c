...
int intel_register_audio_clients(struct snd_sof_dev *sdev)
{
	...
	sdev->sof_mfd_clients = devm_kzalloc(sdev->dev, size, GFP_KERNEL);
	if (!sdev->sof_mfd_clients)
		return -ENOMEM;
...
EXPORT_SYMBOL(hda_register_clients);
...
int intel_unregister_audio_clients(struct snd_sof_dev *sdev)
{
	...
	for (i = 0; i < sdev->sof_num_clients; i++)
		sof_client_dev_unregister(sdev->sof_mfd_client[i]);

...
static int sof_kcontrol_setup(struct snd_sof_dev *sdev, struct snd_sof_control *scontrol)
{
	...
	case SOF_CTRL_CMD_SWITCH:
		...
		ret = snd_sof_ipc_set_get_comp_data(scontrol, ipc_cmd, ctrl_type, scontrol->cmd,
						    true);
		break;
...
static int sof_dai_config_setup(struct snd_sof_dev *sdev, struct snd_sof_dai *dai)
{
	...
	/*
	 * The link DMA channel would be invalidated for running streams but not for streams that
	 * were in the PAUSED state during suspend. So invalidate it here before setting the dai
	 * config in the DSP.
	 */
...
static int sof_widget_setup(struct snd_sof_dev *sdev, struct snd_sof_widget *swidget)
{
	...
	if (ret < 0) {
		dev_err(sdev->dev, "error: failed to enable target core: %d\n", ret);
		return ret;
	}
	...
	if (ret < 0)
		dev_err(sdev->dev, "error: failed to load widget type %d with ID: %d\n",

				    ...
				    const struct sof_dev_desc *desc)
{
	...
	sof_audio->machine->tplg_filename_prefix = desc->default_tplg_path;

	/* use platform-specific machine driver if mach is null */
	if (!sof_mach_get_machine(sof_audio->machine)) {
		sof_mach = snd_sof_client_machine_driver_select(&pdev->dev);
		if (!sof_mach) {
			dev_err(&pdev->dev,
		...
		sof_audio->machine->tplg_filename = sof_mach->tplg_filename;

		/* set machine info */
		switch (sof_audio->machine->type) {
		case SND_SOC_SOF_MACH_TYPE_ACPI:
			/* set machine info */
			sof_mach_set_machine(sof_audio->machine,
					     sof_mach->acpi);

			/* set platform name */
			mach = sof_audio->machine->acpi;
			mach->mach_params.platform = dev_name(&pdev->dev);
			break;
		case SND_SOC_SOF_MACH_TYPE_OF:
			sof_mach_set_machine(sof_audio->machine, sof_mach->of);
			break;
		default:
			return -EINVAL;
		}
...
static int sof_audio_probe(struct platform_device *pdev)
{
	...
	snd_sof_ipc_rx_register(audio_client);
	...
	pdev_mach = sof_audio->machine->pdev_mach;
	pdev_mach = platform_device_register_data(&pdev->dev, drv_name,
						  PLATFORM_DEVID_NONE,
						  machine, size);

	if (IS_ERR(pdev_mach)) {
		ret = PTR_ERR(pdev_mach);
		return ret;
	}

	dev_dbg(&pdev->dev, "created machine %s\n",
		dev_name(&pdev_mach->dev));
...
static int sof_audio_remove(struct platform_device *pdev)
{
	...
	if (!IS_ERR_OR_NULL(sof_audio->machine->pdev_mach))
		platform_device_unregister(sof_audio->machine->pdev_mach);

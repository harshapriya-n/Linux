			      ...
			      struct snd_soc_dobj *dobj)
{
	...
	kfree(scontrol);
	/* send IPC to the DSP */
	return sof_client_tx_message(scomp->dev, fcomp.hdr.cmd, &fcomp,
				     sizeof(fcomp), NULL, 0);
}
			  ...
			  struct sof_ipc_comp_reply *r)
{
	...
	ret = sof_client_tx_message(dev, pipeline->hdr.cmd, pipeline,
	...
	ret = snd_sof_client_core_power_up(dev, 1 << pipeline->core);
	if (ret < 0) {
	...
	core_mask = snd_sof_dsp_get_enabled_cores_mask(dev);
	core_mask |= 1 << pipeline->core;
	snd_sof_dsp_set_enabled_cores_mask(dev, core_mask);
	...
	/* send ipc */
	ret = sof_client_tx_message(dev, pm_core_config.hdr.cmd,
			    ...
			    struct snd_soc_tplg_dapm_widget *tw)
{
	...
	case snd_soc_dapm_dai_out:
		...
		if (ret == 0) {
			...
			list_add(&dai->list, &sof_audio->dai_list);
			swidget->private = dai;
		} else {
			kfree(dai);
		}
			     ...
			     struct snd_soc_dobj *dobj)
{
	...
	case snd_soc_dapm_scheduler:
		...
		ret = snd_sof_client_core_power_down(scomp->dev,
						     1 << pipeline->core);
		if (ret < 0)
			      ...
			      struct sof_ipc_dai_config *config)
{
	 ...
	 */
	if (!found) {
		dev_warn(scomp->dev, "warning: failed to find dai for dai link %s",
			 link->name);
	}
			     ...
			     struct sof_ipc_dai_config *config)
{
	...
	/* send message to DSP */
	ret = sof_client_tx_message(scomp->dev, config->hdr.cmd, config,
	...
	/* set config for all DAI's with name matching the link name */
	ret = sof_set_dai_config(scomp, size, link, config);
	if (ret < 0)
			      ...
			      struct sof_ipc_dai_config *config)
{
	 ...
	 */
	sof_audio->private = kzalloc(sizeof(u32), GFP_KERNEL);
	if (!sof_audio->private) {
		kfree(ipc_config);
		return -ENOMEM;
	}
	...
	/* send message to DSP */
	ret = sof_client_tx_message(scomp->dev, ipc_config->hdr.cmd, ipc_config,
				    size, &reply, sizeof(reply));
	...
	/* set config for all DAI's with name matching the link name */
	ret = sof_set_dai_config(scomp, size, link, ipc_config);
	if (ret < 0)
		dev_err(scomp->dev, "error: failed to save DAI config for DMIC%d\n",
			config->dai_index);

err:
	kfree(sof_audio->private);
	kfree(ipc_config);
			     ...
			     struct sof_ipc_dai_config *config)
{
	...
	/* send message to DSP */
	ret = sof_client_tx_message(scomp->dev, config->hdr.cmd, config, size,
				    &reply, sizeof(reply));
...
static int sof_link_hda_unload(struct snd_soc_component *scomp,
			       struct snd_soc_dai_link *link)
{
	struct snd_soc_dai *dai;
	int ret = 0;
...
found:
	...
	case SOF_DAI_INTEL_ALH:
		...
		break;
	case SOF_DAI_INTEL_HDA:
		ret = sof_link_hda_unload(scomp, link);
		break;
	default:
		dev_err(scomp->dev, "error: invalid DAI type %d\n",
			      ...
			      struct snd_sof_widget *swidget)
{
	...
	ret = sof_client_tx_message(dev, ready.hdr.cmd, &ready,
